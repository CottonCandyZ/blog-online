...
感觉好久没发 Blog，实际上有一篇一直在准备中，关于实现 HoverCard，先放个预览图地说，本篇还在写：

<video width="800" controls playsInline>
  <source src="/images/2024-08-12-global-hover-card/hover-pop-card.webm" type="video/webm" />
</video>
...
<video width="800" controls playsInline>
  <source src="/images/2024-08-12-global-hover-card/remake.webm" type="video/webm" />
</video>

## Hover Card

我想在聊 hover pop card 之前先发一篇聊聊 [hover-card](https://ui.shadcn.com/docs/components/hover-card)，由于 bangumi-electron 一直用 shadcn 作为 “组件” 库，这里组件打引号的原因是因为 shadcn 不像是个完整的组件库，而是一个整合方案。其大部分“组件”都取自于 [radix-ui](https://www.radix-ui.com/)，然后配合 tailwind 形成一个较为一体的设计方案。所以自然当我想要点格字时就会考虑用上 hover-card:
...
<video width="800" controls playsInline>
  <source src="/images/2024-08-12-global-hover-card/radix-ui.webm" type="video/webm" />
</video>
...
>) {
  ...
  return (
    <motion.div
      ref={ref}
      className={cn(
        'bg-popover text-popover-foreground fixed z-50 w-64 rounded-md border p-4 shadow-md outline-none',
        className,
      )}
      // 定位到新的位置
      animate={{
...
if (position === null) setPosition({ ...c }) // [!code focus]
else setTranslate({ X: c.X - position.X, Y: c.Y - position.Y }) // [!code focus]
```

这里实际上我们首先会先计算一个 top, left 用于 fixed 定位，后续的 re-render 我们需要使用 Transition，这样的好处是可以使用 Transition 做一些动画。然后是一个 resize 的 ob：

```ts
useLayoutEffect(() => {
  if (!ref.current) return
  calc()
  const ob = new ResizeObserver(() => { // [!code focus]
    calc() // [!code focus]
  }) // [!code focus]
  ob.observe(ref.current)
  return () => {
