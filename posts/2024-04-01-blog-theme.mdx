...
就像我上面说的那样，一切涉及到用本地数据 init State 的，包括我之前的讨论 Tags 时[说到的](/2024-03-29-blog-tags)，这个问题暂时可以先放在 `useEffect` 中，或者使用 hack 的方法注入 `script` 的 tag。最后这种方法我觉得只是临时的解决方案，因为这个问题应该会改进，毕竟 toggle 主题实际上非常常用。而改进的方案应该将这种 init State 的操作延后到客户端执行，且需要在 DOM 之前。
